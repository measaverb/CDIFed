...
class BottleneckD(nn.Module):
    ...
    def __init__(
        self, in_planes, planes, step_size, learnable_step=True, stride=1, base_width=64
    ):
    ...
    def __init__(self, in_planes, planes, stride=1, track=False):
        ...
        if stride != 1 or in_planes != self.expansion * planes:
            self.downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(
                    self.expansion * planes, momentum=None, track_running_stats=track
                ),
            )
    ...
    def __init__(
        ...
        if step_size:
            ...
            self.step_size = nn.Parameter(
                torch.ones(1, requires_grad=False) * step_size
            )
            self.step_size.requires_grad = False
