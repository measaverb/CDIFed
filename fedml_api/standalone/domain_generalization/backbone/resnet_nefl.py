...
def resnet10wd(
    step_size_2d_list, p, learnable_step, num_classes
):  ## NeFL-WD (kernel 3)
    model = ResNet_WD(
        BasicBlockM,
        [1, 1, 1, 1],
...
def resnet18_HeteroFL(num_classes, p):  # resnet18hf
    model = ResNet_HeteroFL(
        block=BasicBlockH, num_blocks=[2, 2, 2, 2], p_drop=p, num_classes=num_classes
    )
    return model
...
def resnet34_HeteroFL(num_classes, p):  # resnet34hf
    model = ResNet_HeteroFL(
        block=BasicBlockH, num_blocks=[3, 4, 6, 3], p_drop=p, num_classes=num_classes
    )
    return model


def resnet56_HeteroFL(num_classes, p):
    model = ResNet_c_HeteroFL(
        block=BasicBlockH, num_blocks=[9, 9, 9], p_drop=p, num_classes=num_classes
    )
    return model
...
def resnet110_HeteroFL(num_classes, p):
    model = ResNet_c_HeteroFL(
        block=BasicBlockH, num_blocks=[18, 18, 18], p_drop=p, num_classes=num_classes
    )
    return model
...
def resnet101_2_HeteroFL(numclasses, p):
    model = ResNet_HeteroFL(
        block=BottleneckH, num_blocks=[3, 4, 23, 3], p_drop=p, num_classes=num_classes
    )
    return model
