...
class ResNet_HeteroFL(nn.Module):  #  ResNethp, Width-varying ResNet
    def __init__(self, block, num_blocks, p_drop, num_classes=10, track=False):
        ...
        self.bn1 = nn.BatchNorm2d(
            self.in_planes, momentum=None, track_running_stats=track
        )
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        self.layer1 = self._make_layer(
            block, up(64 * p_drop), num_blocks[0], stride=1, track=track
        )
        self.layer2 = self._make_layer(
            block, up(128 * p_drop), num_blocks[1], stride=2, track=track
        )
        self.layer3 = self._make_layer(
            block, up(256 * p_drop), num_blocks[2], stride=2, track=track
        )
        self.layer4 = self._make_layer(
            block, up(512 * p_drop), num_blocks[3], stride=2, track=track
        )
...
class ResNet_c_HeteroFL(
    nn.Module
):  # ResNetchp, Width-varying ResNet designed for CIFAR-10, less parameters with deeper network
    def __init__(self, block, num_blocks, p_drop, num_classes=10, track=False):
        super(ResNet_c_HeteroFL, self).__init__()
        self.in_planes = up(16 * p_drop)

        self.conv1 = nn.Conv2d(
            3, up(16 * p_drop), kernel_size=3, stride=1, padding=1, bias=False
        )
        self.bn1 = nn.BatchNorm2d(
            self.in_planes, momentum=None, track_running_stats=track
        )
        self.layer1 = self._make_layer(
            block, up(16 * p_drop), num_blocks[0], stride=1, track=track
        )
        self.layer2 = self._make_layer(
            block, up(32 * p_drop), num_blocks[1], stride=2, track=track
        )
        self.layer3 = self._make_layer(
            block, up(64 * p_drop), num_blocks[2], stride=2, track=track
        )
